...
export default function AppsPage() {
  ...
  const handleToggleAutoReply = (appId: string) => {
    setApps(apps.map(app =>
      app.id === appId
        ? { ...app, autoReplyEnabled: !app.autoReplyEnabled }
        : app
    ));
  };
  ...
  const handleSyncApp = (appId: string) => {
    setApps(apps.map(app =>
      app.id === appId
        ? { ...app, status: "syncing" as const }
        : app
    ));
    // TODO: Call API to sync app
    setTimeout(() => {
      setApps(prevApps => prevApps.map(app =>
        app.id === appId
          ? { ...app, status: "active" as const, lastSync: "Just now" }
          : app
      ));
    }, 2000);
  };
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="h-14 w-14 rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center shrink-0">
                      {app.icon ? (
                        ...
                        <img src={app.icon} alt={app.name} className="h-10 w-10 rounded-lg" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-0.5">{app.packageName}</p>
                        </div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-4">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="font-medium">{app.totalReviews}</span>
                          <span className="text-muted-foreground">reviews</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                          <Clock className="h-4 w-4 text-muted-foreground" />
                          <span className="font-medium">{app.pendingReplies}</span>
                          <span className="text-muted-foreground">pending</span>
                        </div>
                        {app.avgRating > 0 && (
                          <div className="flex items-center gap-1 text-sm">
                            <Star className="h-4 w-4 text-amber-500 fill-amber-500" />
                            <span className="font-medium">{app.avgRating.toFixed(1)}</span>
                          </div>
                        )}
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <Switch
                        ...
                        checked={app.autoReplyEnabled}
                        onCheckedChange={() => handleToggleAutoReply(app.id)}
                      />
                    </div>
                  </div>
                </CardContent>
